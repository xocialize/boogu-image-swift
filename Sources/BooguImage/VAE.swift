// FLUX.1 AutoencoderKL — Swift mirror of boogu_image_mlx/models/vae.py.
//
// Isomorphic with diffusers AutoencoderKL (module/param names preserved) so the
// checkpoint maps key-for-key (244 tensors); only Conv2d `.weight` needs the
// (O,I,H,W)->(O,H,W,I) transpose, done at load (Weights.swift). 16 latent channels,
// block_out_channels [128,256,512,512], 2 enc / 3 dec resnets, GroupNorm(32), SiLU,
// single-head mid attention, NO quant / post-quant convs. All convs run NHWC; tensors
// enter/exit encode()/decode() in NCHW (diffusers convention) and transpose at the edge.
...
public final class AutoencoderKL: Module {
    ...
    public init(_ cfg: BooguVAEConfig, eps: Float = 1e-6) {
